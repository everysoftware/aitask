...
from aitask.users.config import auth_settings
from aitask.users.models import User
from aitask.users.repositories import TelegramUserSpecification
from aitask.users.schemas import (
    UserCreate,
)
...
class AuthUseCases(UseCase):
    ...
    async def get_by_telegram_id(self, telegram_id: int) -> User | None:
        return await self.uow.users.find(TelegramUserSpecification(telegram_id))

    async def get_one_by_telegram_id(self, telegram_id: int) -> User:
        return await self.uow.users.find_one(TelegramUserSpecification(telegram_id))
    ...
    async def register(self, state: FSMContext, data: UserCreate) -> User:
        ...
        await state.update_data(workspace_id=str(workspace.id), todo_list_id=str(todo_list.id))
        return user
